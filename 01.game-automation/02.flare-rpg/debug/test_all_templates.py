...
def find_template(screen, template_name, threshold=0.8):
    ...
    if template is None:
        print(f"[{template_name}] ❌ 로드 실패")
        return False
    result = cv2.matchTemplate(screen, template, cv2.TM_CCOEFF_NORMED)
    _, max_val, _, _ = cv2.minMaxLoc(result)
    print(f"[{template_name}] 유사도: {max_val:.3f} → {'✅ 발견!' if max_val >= threshold else '❌ 미발견'}")
    return max_val >= threshold

# 메인메뉴 상태에서 실행
screen = capture_game()
find_template(screen, "logo.png")
