# 역할: Flare RPG 게임 화면 캡처와 템플릿 매칭 관련 유틸리티 함수 모음
...
def capture_game():
    ...
    if not hwnd:
        raise Exception(f"{WINDOW_TITLE} 창을 찾을 수 없음")
...
def find_template(screen, template_name, threshold=0.8):
    ...
    - template_name: 찾을 템플릿 파일명 (예: "logo.png")
...
def find_template_location(screen, template_name, threshold=0.8):
    """
    템플릿 위치의 중앙 좌표 반환
    클릭 좌표 하드코딩 없이 템플릿으로 자동 위치 탐지

    매개변수:
    - screen: capture_game()으로 캡처한 게임 화면 이미지
    - template_name: 찾을 템플릿 파일명
    - threshold: 유사도 기준값 (0~1, 기본값 0.8)
...
def wait_for_pixel_change(get_area_func, timeout=15, interval=0.5, threshold=1.0):
    """
    특정 영역의 픽셀 변화가 감지될 때까지 대기

    매개변수:
    - get_area_func: 영역을 반환하는 함수 (예: page.get_player_hp_area)
    - timeout: 최대 대기 시간 (초)
    - interval: 체크 간격 (초)
    - threshold: 변화 감지 임계값
