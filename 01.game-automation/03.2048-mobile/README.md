...
uiautomator2 + pytest 기반 모바일 자동화 테스트를 구현했습니다.

에뮬레이터와 실기기(갤럭시 Z 플립3) 양쪽에서 **동일한 코드로 검증**했으며,
실패 시 Jira 티켓 자동 생성과 Slack 알림까지 연결한 포트폴리오 프로젝트입니다.
...
이 프로젝트의 핵심 설계 판단은 **"무엇을 검증하느냐에 따라 수단을 다르게 선택한 것"** 입니다.

앱 UI 계층을 덤프한 결과, 점수·버튼 등은 `resource-id`로 접근 가능했지만
게임 보드의 개별 타일은 커스텀 뷰로 렌더링되어 UI 속성에 노출되지 않았습니다.
...
이미지 비교는 **UI 속성으로 검증할 수 없을 때만** 사용한다는 원칙을 지켰습니다.
...
동일 포트폴리오의 [Flare RPG 프로젝트](../02.flare-rpg)와 대비하면 접근 방식의 차이가 명확합니다.
...
동일한 테스트 코드로 양쪽 환경 모두 전체 통과했으며,
환경변수만 변경하여 대상을 전환합니다.
...
GitHub Actions self-hosted runner에서 자동 실행됩니다.
