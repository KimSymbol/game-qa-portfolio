...
QA 테스트에 필요한 **더미 데이터 9종**을 자동으로 찍어내는 도구.
한국어·영어 faker에 게임 도메인 데이터를 섞어 그럴듯한 데이터를 만든다.
...
생성된 데이터는 다른 도구의 입력으로 바로 사용할 수 있다.
...
이 도구는 **쉽게 확장하거나 수정**할 수 있도록 설계했다.

### 1. 데이터 풀 수정 (이름, 직업, 맵 등)

`generator.py` 상단의 데이터 풀을 수정하면 모든 함수에 자동 반영된다.
...
`generator.py` 의 `CSV_쓰기` 함수만 수정하면 모든 데이터에 일괄 적용된다.
