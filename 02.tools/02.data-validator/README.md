...
CSV/XLSX 파일의 **데이터 무결성을 자동으로 검사**하는 도구입니다.
JSON 규칙 파일로 검사 항목을 관리하며, 규칙 파일이 없어도 기본 검사가 가능합니다.
...
새 데이터의 검증 규칙을 만들 때 사용할 JSON 템플릿을 생성합니다.
...
특정 컬럼을 필수에서 제외하거나 추가할 수 있습니다.

예시: "발견자" 를 필수에서 제외

1. `rules/testcases.json` 열기
2. `필수_컬럼` 리스트에서 "발견자" 제거
3. `컬럼_규칙` 에서 `"필수": false` 로 변경
...
`rules/bugs.json` 의 허용값만 수정하면 됩니다.
