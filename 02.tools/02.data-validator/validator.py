...
def 규칙_템플릿_생성():
    """
    새 검증 규칙을 만들 때 사용할 빈 JSON 템플릿 생성
    rules/ 폴더에 template.json 으로 저장

    사용법:
    1. template.json 복사해서 이름 변경 (예: quests.json)
    2. 필수_컬럼, 컬럼_규칙 채우기
    3. validator.py 의 매핑목록에 키워드 추가
    """
    규칙폴더 = 기준경로 / "rules"
    규칙폴더.mkdir(exist_ok=True)
    ...
    파일명 = 규칙폴더 / "template.json"

    with open(파일명, "w", encoding="utf-8") as f:
        json.dump(템플릿, f, ensure_ascii=False, indent=4)

    log.info(f"규칙 템플릿 생성: {파일명}")
    return 파일명