...
버그 데이터를 **5시트 엑셀 + JSON + HTML + PDF** 4가지 형식으로 동시 변환하는 통합 리포트 도구.
...
외부 데이터에 필수 컬럼이 없어도 기본값을 자동으로 채워서 동작한다.

| 누락 컬럼 | 기본값 |
|----------|--------|
| 버그ID | BUG-001 부터 자동 생성 |
| 제목 | "제목 없음" |
| 심각도 | Medium |
| 우선순위 | Medium |
| 상태 | 미해결 |
...
```bash
# 컬럼이 3개만 있는 파일도 정상 동작
python main.py minimal_bugs.csv --all
```

### 필수 컬럼 기본값 변경

`reporter.py` 의 `데이터_읽기()` 함수 안 `필수컬럼_기본값` 딕셔너리를 수정.

#### 기본값 변경 (예: 플랫폼 기본값을 Android 로)

```python
필수컬럼_기본값 = {
    "플랫폼"  : "Android",    # PC → Android 로 변경
    ...
}
```

#### 특정 컬럼 자동 추가 안 하기 (예: "발견자" 제외)
...
필수컬럼_기본값 = {
    ...
    "상태"    : "미해결",
    "플랫폼"  : "PC",
    # "발견자" : "",       ← 주석 처리하면 자동 추가 안 됨
    ...
}
```

주의: 완전히 제거하면 해당 컬럼 없는 데이터에서 에러 날 수 있다.
안전하게 빈 값(`""`)으로 유지하는 것을 권장.
