...
게임 QA 업무 자동화를 위한 6가지 파이썬 도구 모음과 통합 파이프라인.

ISTQB 기반 테스트 케이스 생성부터 데이터 검증, 여러 형식의 리포트 출력까지 — QA 워크플로우에서 손이 많이 가던 일을 하나의 흐름으로 묶어 자동화했다.
...
```
+---------------------------------------+
|  pipeline.py (Integrated Automation)  |
+---------------------------------------+
                   |
                   v
...
+-----------------------+
  QA + 기획 더미 데이터 9종
  bugs / testcases / logs / items / skills / monsters / ...
...
| Data Validation       |
+-----------------------+
  JSON 규칙 기반 데이터 무결성 검증
  8가지 검사: 필수컬럼 / 빈값 / 허용값 / 날짜 / 중복 / 패턴 / 범위 / 공백
...
| Markdown Report       |
+-----------------------+
  Fail 케이스 → 마크다운 버그 리포트
  개별 .md + 통합 ALL_BUGS.md
...
각 도구에서 빈 파일을 생성해 직접 데이터를 입력할 수 있다.
...
다른 팀/프로젝트의 TC나 버그 데이터를 내부 형식으로 변환할 수 있다.
...
자세한 사용법과 변환 예시는 [common (공통 모듈)](./common/README.md) 참고.

## 누락 컬럼 자동 보완

외부 데이터에 필수 컬럼이 없어도 도구가 자동으로 기본값을 채워서 동작한다.
...
특정 컬럼을 필수에서 제외하거나 기본값을 변경할 수 있다.
...
각 도구를 실행하면 도구별 `결과/` 폴더에 결과물이 저장된다.
...
> 💡 각 파일은 **타임스탬프 + latest** 두 가지 형식으로 자동 생성된다.

---

## 📝 개별 도구 사용법

각 도구의 자세한 사용법은 해당 폴더의 `README.md` 참고.
